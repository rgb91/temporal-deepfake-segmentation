...
def build_model(input_shape, head_size, num_heads, ff_dim, num_transformer_blocks, mlp_units, n_classes, dropout=0.0,
    ...
    # x = layers.GlobalAveragePooling1D(data_format="channels_first")(x)  # output: (None, 5)
    x = layers.GlobalAveragePooling1D()(x)  # output: (None, 768)
    # x = layers.Flatten()(x)  # output: (None, 3840)
    # print(x.shape)
    # exit(1)
...
if __name__ == '__main__':
    ...
    DATA_ROOT_TRAIN = os.path.join(DATA_ROOT, 'FFpp_train_embeddings_npy_batches_10steps_binary_overlap')
    DATA_ROOT_TEST = os.path.join(DATA_ROOT, 'FFpp_test_embeddings_npy_batches_10steps_binary_overlap')
    ...
    model = build_model(input_shape, head_size=512, num_heads=8, ff_dim=8, num_transformer_blocks=8,
                        mlp_units=[512], n_classes=N_CLASSES, mlp_dropout=0.4, dropout=0.25, masking=False)
