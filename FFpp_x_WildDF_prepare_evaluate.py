...
from utils import split_dataframe, make_short_file
...
def correct_filename_column(infile, outfile):
    df_chunks = pd.read_csv(infile,
                            chunksize=10000,
                            low_memory=False,
                            index_col=False,
...
def make_npy_by_video_FFpp_x_WildDF(infile, out_dir, timesteps=5, overlap=0):
    class_name_num_map = {'real': 0, 'fake': 1}
    ...
    for video_name, filtered_df in tqdm(grouped_df):
        ...
        # print(df_f.head())
        # df_f.to_csv(os.path.join(out_dir, video_name + '.csv'), header=False, index=False)
...
def save_folder_count(infile, outfile):
    df_chunks = pd.read_csv(infile,
                            chunksize=10000,
                            low_memory=False,
                            index_col=False,
                            header=0)
    df = pd.concat(df_chunks)
    df2 = pd.DataFrame([Counter(df['folder'])])
    df2.T.to_csv(outfile)


if __name__ == '__main__':
    DATA_ROOT = r'/data/PROJECT FILES/DFD_Embeddings/FFpp_embeddings/cross/WildDF'

    # clean_raw_csv(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings.csv'),
    #               outfile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned.csv'))

    # make_short_file(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned.csv'),
    #                 outfile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned_short.csv'),
    #                 use_pandas=True,
    #                 sample_size=500)

    # correct_filename_column(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned.csv'),
    #                         outfile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned_.csv'))

    # save_predictions_only(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned.csv'),
    #                       outfile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_predictions.csv'))

    # save_folder_count(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_predictions.csv'),
    #                   outfile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_folders.csv'))

    # evaluate_ViT(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_predictions.csv'))

    make_npy_by_video_FFpp_x_WildDF(infile=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_test_embeddings_cleaned.csv'),
                                    out_dir=os.path.join(DATA_ROOT, 'FFpp_x_WildDF_npy_videos_5steps'),
                                    timesteps=5,
                                    overlap=0)